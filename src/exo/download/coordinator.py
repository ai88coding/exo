        ...
        for target in rsync_targets.split(","):
            ...
            try:
                ...
                if proc.returncode == 0:
                    ...
                    logger.warning(
                        f"scp to {target} failed (exit {proc.returncode}): {proc.stderr[:200]}"
                    )
            except Exception as exc:
                logger.warning(f"scp to {target} error: {exc}")
    ...
    async def _start_download(self, shard: ShardMetadata) -> None:
        ...
        if model_id in self.download_status:
            ...
            if isinstance(status, (DownloadOngoing, DownloadCompleted)):
                ...
                return
            # DownloadFailed or DownloadPending — allow retry
        ...
        if found_path is not None:
            ...
            if os.environ.get("EXO_MODEL_SOURCE") != "rsync-only":
                await self._sync_to_peers(model_id, found_path)
            return

        # If rsync-only mode (sparka/sparkb), pull from Mac and re-check before giving up
        if os.environ.get("EXO_MODEL_SOURCE") == "rsync-only":
            logger.info(f"rsync-only mode: pulling {model_id} from Mac")
            ...
            if src:
                ...
                for source_path in source_paths:
                    logger.info(f"Attempting to pull model from {source_path}")
                    # Construct the specific model path on the remote Mac (with trailing slash to copy contents)
                    remote_model_path = f"{source_path}/"
                    # The destination is the default exo models directory for this model_id (normalized)
                    model_subpath = str(EXO_DEFAULT_MODELS_DIR / model_id.normalize())
                    try:
                        proc = await to_thread.run_sync(
                            lambda: subprocess.run(
                                ["scp", "-r", remote_model_path, f"{model_subpath}/"],
                                capture_output=True,
                                text=True,
                                timeout=3600,
                            )
                        ...
                        if proc.returncode == 0:
                            logger.info(f"scp pull succeeded for {model_id} from {source_path}")
                            pulled_successfully = True
                            break
                        else:
                            logger.warning(f"scp pull failed from {source_path}: {proc.stderr[:200]}")
                    except Exception as e:
                        logger.warning(f"scp pull error from {source_path}: {e}")
                if not pulled_successfully:
                    logger.warning(f"rsync pull failed for {model_id} from all attempted sources")
                    # Emit failed status so dashboard shows error
                    failed = DownloadFailed(
                        shard_metadata=shard,
                        node_id=self.node_id,
                        error_message=f"Model {model_id} not found on Mac at attempted sources: {source_paths}",
                        model_directory=self._default_model_dir(model_id),
                    )
                    self.download_status[model_id] = failed
                    await self.event_sender.send(NodeDownloadProgress(download_progress=failed))
                    return
            # Re-check after rsync
            found_path = await to_thread.run_sync(
                resolve_existing_model, model_id, shard.model_card
            )
            if found_path is not None:
                logger.info(f"DownloadCoordinator: {model_id} found after rsync pull")
                completed = self._completed_from_path(
                    shard, found_path, shard.model_card.storage_size
                )
                self.download_status[model_id] = completed
                ...
                return
            logger.info(f"rsync-only mode: {model_id} not available from Mac, deferring")
            # Emit failed status so dashboard shows error
            failed = DownloadFailed(
                shard_metadata=shard,
                node_id=self.node_id,
                error_message=f"Model {model_id} not found on Mac after rsync pull",
                model_directory=self._default_model_dir(model_id),
            )
            self.download_status[model_id] = failed
            await self.event_sender.send(NodeDownloadProgress(download_progress=failed))
            ...
            try:
                ...
                async for (
                    ...
                    if progress.status == "complete":
                        ...
                        # Don't downgrade a model that is already confirmed complete or failed.
                        if isinstance(
                            self.download_status.get(model_id), (DownloadCompleted, DownloadFailed)
                        ):
                            continue
